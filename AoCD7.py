...
# abba[mnop]qrst[mndi]zsdfr"""

# INPUT = "ioxxoj[asdfgh]zxcvbn"

def check_string(text):
    reversed_text = text[2:][1] + text[2:][0]
    if text[:2] == reversed_text and text[:2] != text[2:]:
        return True
    return False

def is_palindrome(text):
    for pos in range(0, len(text)):
        if len(text[pos:pos+4]) == 4 and check_string(text[pos:pos+4]):
            return True
    return False
...
for item in INPUT.split('\n'):
    ...
    for thing in front:
        if thing.find(']') != -1:
            bad_items.append(thing.split(']')[0])
            if is_palindrome(thing.split(']')[1]):
                is_valid = True
        elif is_palindrome(thing):
            is_valid = True
    for bad in bad_items:
        if is_palindrome(bad):
            is_valid = False
    if is_valid:
        num_valid += 1
print(num_valid)
...
def find_the_thing(text):
    to_return = []
    for pos in range(0, len(text)):
        if len(text[pos:]) >= 3 and text[pos] == text[pos+2]:
            to_return.append(text[pos:pos+3])
    return to_return
...
for item in INPUT.split('\n'):
    ...
    for thing in front:
        if thing.find(']') != -1:
            inners += find_the_thing(thing.split(']')[0])
            outers += find_the_thing(thing.split(']')[1])
        else:
            outers += find_the_thing(thing)
    for inner in inners:
        inverse = inner[1] + inner[0] + inner[1]
        if inverse in outers:
            is_valid = True
    if is_valid:
        num_valid += 1
print(num_valid)