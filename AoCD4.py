# Advent of Code, D4_2016

INPUT_STRING = """vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]
...
# INPUT_STRING = """aaaaa-bbb-z-y-x-123[abxyz]
# a-b-c-d-e-f-g-h-987[abcde]
# not-a-real-room-404[oarel]
# totally-real-room-200[decoy]"""

def find_most_common(num_common, check):
    chars = {}
    for char in check:
        if char in chars:
            chars[char] += 1
            ...
            chars[char] = 0
    chars_sorted = [v[0] for v in sorted(chars.items(), key=lambda val: (-val[1], val[0]))]
    return ''.join(chars_sorted[:num_common])

def get_value_and_name(combined):
...
def get_sum_of_valid_rooms():
    total = 0
    for row in INPUT_STRING.split('\n'):
        input_split = row.split('-')
        checksum = ''.join(input_split[0:len(input_split)-1])
        encrypted_name = ''.join(input_split[len(input_split)-1:])
        most_common = find_most_common(5, checksum)
        val_name = get_value_and_name(encrypted_name)
        if most_common == val_name[1]:
            total += int(val_name[0])
    return total

def find_north_pole_room():
    sector_id = 0
    for row in INPUT_STRING.split('\n'):
        hash_str = row.split('[')[0].split('-')[:-1]
        input_split = row.split('-')
        encrypted_name = ''.join(input_split[len(input_split)-1:])
        val = int(get_value_and_name(encrypted_name)[0])
        unencrypted_name = figure_name(hash_str, val)
        if unencrypted_name.find("north") != -1:
            sector_id = val
            break
    return sector_id

print(get_sum_of_valid_rooms())
print(find_north_pole_room())
